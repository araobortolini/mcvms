...
import { LayoutDashboard, Users, Store, Settings, LogOut, BarChart3, Wallet, LifeBuoy } from "lucide-react";
...
export function Sidebar() {
  ...
  const masterLinks = [
    ...
    { name: "Chamados", href: "/dashboard/master/reports", icon: LifeBuoy },
  ...
  const resellerLinks = [
    ...
    { name: "Suporte", href: "/dashboard/reports", icon: LifeBuoy },
    ...
    <div className="flex h-full flex-col bg-white border-r px-4 py-6">
      ...
      <nav className="flex-1 space-y-1">
        {activeLinks.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "text-gray-600 hover:bg-gray-50"}`}>
      ...
      {!isImpersonating && (
        <button onClick={() => signOut()} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors mt-auto">
